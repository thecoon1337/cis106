## Notes 8

## `cat` 
- **Used to display the content of a file.**

**Formula:** `cat` + `option` + `file(s) to display`

**Example:** `cat` + `~/Documents/sample_files/file.txt`

## `tac`
- **Used to display the content of a file in reverse order.**

**Formula:** `tac` + `option` + `file(s) to display`

**Example:** `tac` + `~/Documents/sample_files/file.txt`

## `head`
- **Displays the first N number of lines in a given file. Prints the first 10 by default.**

**Formula:** `head` + `option` + `files(s) to display`

**Example:** `head` + `~/Documents/sample_files/file.txt`

## `tail`
- **Displays the last N number of lines in a given file. Prints the last 10 by default.**

**Formula:** `tail` + `option` + `file(s) to display`

**Example:** `tail` + `~/Documents/sample_files/file.txt`

## `cut`
- **Used  to remove a specific section of each line in a file and display it.**

**Formula:** `cut` + `option` + `file(s) to display`

**Example:** `cut` + `-d ':' -f1 /etc/passwd/`

## `sort`
- **Used to sort files. Can sort alphabetically, in reverse, by number and by date.**

**Formula:** `sort` + `option` + `file(s) to display`

**Example:** `sort` + `file.txt`

## `wc`
- **Used to print the number of lines characters and bytes in a file.**

**Formula:** `wc` + `option` + `file(s) to display`

**Example:** `wc` + `l` + `file.txt`  